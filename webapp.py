...
for d in ["inputs/current","inputs/lastweek","inputs/trades",OUT]: os.makedirs(d,exist_ok=True)
app=Flask(__name__)
...
def check_auth(u,p): return secrets.compare_digest(u,WEBAPP_USER) and secrets.compare_digest(p,WEBAPP_PASS)
...
FOLDERS={
 "current":"inputs/current","lastweek":"inputs/lastweek","trades":"inputs/trades",
 "screener_holdings":"inputs","watchlist":"inputs","screener_watchlist":"inputs",
}
SINGLE={"screener_holdings":"screener_holdings.csv","watchlist":"watchlist.csv","screener_watchlist":"screener_watchlist.csv"}

def input_status():
    def n(p): return len([f for f in glob.glob(p) if os.path.basename(f)!=".gitkeep"])
    return {
...
def worker(args):
    with LOCK: JOB.update(running=True,log=[],done=False,rc=None,started=datetime.datetime.now().strftime("%H:%M:%S"))
    try:
        ...
        for line in p.stdout:
            with LOCK: JOB["log"].append(line.rstrip())
        p.wait()
        with LOCK: JOB.update(running=False,done=True,rc=p.returncode)
    except Exception as e:
        with LOCK: JOB["log"].append("ERROR: "+str(e)); JOB.update(running=False,done=True,rc=1)

@app.route("/")
def index(): return Response(PAGE,mimetype="text/html")

@app.route("/inputs")
def inputs(): return jsonify(input_status())

@app.route("/upload",methods=["POST"])
def upload():
    tgt=request.form.get("target","current"); folder=FOLDERS.get(tgt,"inputs/current")
    files=request.files.getlist("files"); saved=[]
    for f in files:
        name=SINGLE[tgt] if tgt in SINGLE else os.path.basename(f.filename)
        f.save(os.path.join(folder,name)); saved.append(name)
    return jsonify(saved=saved,status=input_status())

@app.route("/run",methods=["POST"])
def run():
    with LOCK:
        if JOB["running"]: return jsonify(error="already running"),409
    d=request.get_json(force=True,silent=True) or {}
    args=["--cur",d.get("cur",datetime.date.today().strftime("%d %B %Y")),
          "--prev",d.get("prev",(datetime.date.today()-datetime.timedelta(days=7)).strftime("%d %B %Y")),
          "--fii",d.get("fii","n/a"),"--dii",d.get("dii","n/a"),"--flowdate",d.get("flowdate","latest close")]
...
<title>Weekly Portfolio Kit</title><style>
...
h1{font-size:22px;color:#0f2148}h2{font-size:15px;color:#33415c;margin:22px 0 8px}
...
.ok{color:#1a7f37;font-weight:600}.no{color:#b3261e}
...
<div class=row><div><label>Add to</label><select id=target>
...
</select></div><div><label>Files</label><input type=file id=files multiple></div></div>
<button onclick=up()>Upload</button><div class=hint id=uphint></div></div>
<div class=card><h2>3 · Dates &amp; flows</h2>
<div class=row><div><label>This week</label><input id=cur placeholder="22 August 2026"></div>
<div><label>Last week</label><input id=prev placeholder="25 July"></div></div>
<div class=row><div><label>FII net (Cr)</label><input id=fii placeholder="+1,181.66"></div>
<div><label>DII net (Cr)</label><input id=dii placeholder="+2,493.41"></div>
<div><label>Flow date</label><input id=flowdate placeholder="24 Aug 2026 close"></div></div></div>
<div class=card><h2>4 · Run</h2><button id=runbtn onclick=run()>▶ Run weekly review</button>
<pre id=log></pre></div>
<div class=card id=dlcard style=display:none><h2>5 · Downloads</h2><div id=downloads></div></div>
<script>
async function refresh(){let s=await (await fetch('/inputs')).json();document.getElementById('status').innerHTML=
 Object.entries(s).map(([k,v])=>`<div class=st><span>${k}</span><span class="${v?'ok':'no'}">${v?'✓ '+v:'—'}</span></div>`).join('')}
async function up(){let fd=new FormData();fd.append('target',target.value);
 for(let f of files.files)fd.append('files',f);
 if(!files.files.length){uphint.textContent='pick file(s) first';return}
 uphint.textContent='uploading…';let r=await (await fetch('/upload',{method:'POST',body:fd})).json();
 uphint.textContent='added: '+r.saved.join(', ');files.value='';refresh()}
let timer;
async function run(){runbtn.disabled=true;log.style.display='block';log.textContent='starting…';dlcard.style.display='none';
 let body={cur:cur.value,prev:prev.value,fii:fii.value||'n/a',dii:dii.value||'n/a',flowdate:flowdate.value||'latest close'};
...
async function poll(){let s=await (await fetch('/status')).json();log.textContent=s.log||'working…';log.scrollTop=log.scrollHeight;
 if(s.done){clearInterval(timer);runbtn.disabled=false;
  log.textContent+= s.rc===0?'\n\n✔ done':'\n\n⚠ finished with errors (rc='+s.rc+')';
  if(s.outputs.length){dlcard.style.display='block';
   downloads.innerHTML=s.outputs.map(o=>`<a class=dl href="/download/${encodeURIComponent(o)}">⬇ ${o}</a>`).join('')}}}
refresh()
</script></body></html>"""
if __name__=="__main__":
    # Local/dev only. On a server, run via gunicorn (see Dockerfile/CMD) — not this dev server.
