...
def status(path,label):
    n=len(glob.glob(path)); st.write(("✅" if n else "⬜")+f" {label}: {n} file(s)")
...
if ups:
    for u in ups:
        d="inputs" if tgt.startswith("inputs (") else tgt; os.makedirs(d,exist_ok=True)
...
fii=c3.text_input("FII net (Cr)","n/a"); dii=c4.text_input("DII net (Cr)","n/a"); fd=c5.text_input("Flow date","latest close")
